...
### Access Swagger UI
The project includes comprehensive OpenAPI 3.0 documentation via Swagger UI. Once the application is running, access it at:

**Swagger UI:** http://localhost:2026/swagger-ui/index.html  
**OpenAPI JSON:** http://localhost:2026/v3/api-docs

### API Features
- **Versioning:** All endpoints available at both `/users` and `/api/v1/users` (and same for `/orders`)
- **DTOs:** Request/response bodies use DTOs with validation
- **Pagination:** Page `?page=0&size=20&sort=name,asc` parameters
- **Optimistic Locking:** Version field prevents concurrent update conflicts
- **Caching:** Individual lookups cached for 10 minutes, paginated results for 5 minutes
- **Validation:** Request bodies validated with detailed error feedback
- **HTTP Status Codes:**
...
GET /api/v1/users/1
```
Response (200 OK) — result cached for 10 minutes
...
Invoke-RestMethod -Uri 'http://localhost:2026/api/v1/users' `
  -Method POST `
...
**Get Users:**
```powershell
Invoke-RestMethod -Uri 'http://localhost:2026/api/v1/users?page=0&size=10' -Method GET
```

**Update User:**
```powershell
# Fetch the user to get current version
$user = Invoke-RestMethod -Uri 'http://localhost:2026/api/v1/users/1' -Method GET

# Update the user
$user.name = 'Alice Updated'
$body = $user | ConvertTo-Json
Invoke-RestMethod -Uri 'http://localhost:2026/api/v1/users/1' `
  -Method PUT `
  -Headers @{ 'Content-Type' = 'application/json' } `
  -Body $body
```
...
**Delete User:**
```powershell
Invoke-RestMethod -Uri 'http://localhost:2026/api/v1/users/1' -Method DELETE
```

### API Characteristics

- **Request/Response Bodies:** All endpoints use DTO objects for type safety and documentation
- **Field Validation:** Name (required, max 200 chars), Email (required, valid format), ProductName (required, max 255), Price (required, positive)
- **Caching Strategy:** 
  - Single resource (`getById`) → 10-minute cache (Hazelcast LRU)
  - Paginated results → 5-minute cache
  - Cache invalidated on write operations (create/update/delete)
- **Pagination Defaults:** Page size = 20 if not specified
- **Sorting:** Use `sort=fieldName,asc|desc` parameter (e.g., `sort=name,desc`)
- **Optimistic Locking:** Send the correct `version` field on updates; server returns 409 if version mismatch
